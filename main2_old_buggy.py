...
MANAGE_FIELDS = 7
ADD_FIELD = 8
DELETE_FIELD_SELECT = 9
CHANGE_THEME = 10
UPLOAD_FILE = 11
CONFIRM_DELETE_ALL = 12
...
async def search_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if results.empty:
            ...
            message = format_record_display(results, MAX_DISPLAY_RECORDS, f"🔍 نتایج جستجو برای '{query}':")
...
📈 وضعیت فیلدها:"""
        ...
        for field in fields:
            if field in df.columns:
                ...
                msg += f"\n• {field}: {filled}/{total_records} ({percentage:.0f}%)"
...
async def delete_all_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"Error in delete_all_start: {e}")
...
💡 نکات مفید:
- فایل‌های Excel با رنگ‌بندی زیبا تولید می‌شوند
- تمام عملیات لاگ می‌شوند
- امکان جستجو در تمام فیلدها
...
async def change_theme_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(f"User {update.effective_user.id} changed theme to: {selected_theme}")
